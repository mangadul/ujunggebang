...
class First_Artikel_M extends CI_Model{
	...
	function artikel_show($id='0',$offset,$limit){
		...
		if($query->num_rows()>0){
			...
			while($i<count($data)){
				...
				$pendek = word_limiter($teks, 10);
				$data[$i]['isi_short'] = $pendek;
				$panjang = word_limiter($teks, 50);
	...
	function arsip_show(){
		...
		while($i<count($data)){
			...
			$data[$i]['isi'] = "<label>".$panjang[0]."...</label><a href='".site_url("artikel/$id/".url_title($data[$i]['judul']))."'>baca selengkapnya</a>";
			$i++;
		}
		return $data;
	}
	...
	function full_arsip($offset=0,$limit=50){
		...
		if($query->num_rows()>0){
			...
			while($i<count($data)){
				...
				$data[$i]['no'] = $nomer;
				$data[$i]['tgl'] = $tgl;
				$data[$i]['isi'] = "<a href='".site_url("artikel/$id/".url_title($data[$i]['judul']))."'>".$data[$i]['judul']."</a>, <i class=\"fa fa-user\"></i> ".$data[$i]['owner'];
				$i++;
			}
		}else{
			$data  = false;
	...
	function komentar_show(){
		...
		while($i<count($data)){
			...
			$data[$i]['komentar'] = "".$panjang[0]."...<a href='".site_url("artikel/$id")."'>baca selengkapnya</a>";
			$i++;
		}
		return $data;
	}
