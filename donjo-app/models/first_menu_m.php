	...
	function list_menu_atas(){
		...
		while($i<count($data)){
			...
			if(count($data2)) {
				...
				$data[$i]['menu'] = $data[$i]['menu']. '
                <li class="dropdown">
                  <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">'.$data[$i]['nama'].'<span class="caret"></span></a>
				...
				while($j<count($data2)){
					...
					//if($data2[$j]['link_tipe']!=1)
						$data[$i]['menu'] = $data[$i]['menu']."<li><a href='$url".$data2[$j]['link']."/".url_title($data2[$j]['nama'])."'>".$data2[$j]['nama']."</a></li>";
					//else
